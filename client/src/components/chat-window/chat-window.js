        ...
        <div>
          <div id="reply-div" class="reply-div">
            ...
            text-overflow: ellipsis;">
              <div style="display: flex"><div class=reply-icon></div>You are replying to</div>
              <div style="display:flex; flex-direction:row; margin-top:8px;">
                <div class="divider-left"></div>
                <div style="display: flex;
                flex-direction: column; margin-left: 8px;">
                    <div style="width:100%" class="reply-to-username">${this.toReplyContent.userName}</div>
                    <div style="width:100%; text-overflow: ellipsis;
                    white-space: nowrap;
                    overflow: hidden;" class="reply-to-message">${this.toReplyContent.message}</div>
                </div>
              </div>
            </div>
...
//   <div style="width:100%" class="reply-to-username">${this.toReplyContent.userName}</div>
//                     <div style="width:100%" class="reply-to-message">${this.toReplyContent.message}</div>
  ...
  sendMessageEvent(message) {
    if(message != '' && message != undefined && message != null) {
        ...
        if(this.toReplyContent?.messageId != '' && this.toReplyContent?.messageId != undefined && this.toReplyContent?.messageId != null) {
            console.log("replyContent: ",this.toReplyContent);
            messageObj.replyMessage = true;
            messageObj.messageId = this.toReplyContent?.messageId;
            socketService.getSocket().emit("sendReply", JSON.stringify(messageObj));
        } else {
            socketService.getSocket().emit("sendMessage", JSON.stringify(messageObj));
        }
  ...
  addReply(messageData) {
        ...
        messageData.initials = messageData.firstName?.[0] + messageData.lastName?.[0];
        messageData.messageText = messageData.message;
        console.log("add reply messageData: ",messageData);
        let messageUi = this.shadowRoot.getElementById(""+messageData?.messageId);
        console.log(messageUi);
        messageUi?.addReply(messageData);
